...
public class InventoryCountingBatchService(
    SystemDbContext db,
    IExternalSystemAdapter adapter,
    ISettings settings,
    IExternalSystemAlertService alertService,
    IServiceScopeFactory serviceScopeFactory,
    ILogger<InventoryCountingBatchService> logger) : IInventoryCountingBatchService {
        ...
        try {
            ...
            _ = Task.Run(async () => {
                try {
                    ...
                    var bgService = scope.ServiceProvider.GetRequiredService<IInventoryCountingBatchService>();
                    await bgService.ProcessBatchesInBackground(id, sessionInfo);
                }
                catch (Exception ex) {
                    // Log but don't throw - this is fire-and-forget
                    using var scope = serviceScopeFactory.CreateScope();
                    var bgLogger = scope.ServiceProvider.GetRequiredService<ILogger<InventoryCountingBatchService>>();
    ...
    public async Task<ProcessInventoryCountingResponse> RetryFailedBatches(RetryBatchRequest request, SessionInfo sessionInfo) {
        ...
        _ = Task.Run(async () => {
            try {
                ...
                var bgService = scope.ServiceProvider.GetRequiredService<IInventoryCountingBatchService>() as InventoryCountingBatchService;
