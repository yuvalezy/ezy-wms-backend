...
public class PublicService(IExternalSystemAdapter adapter, ISettings settings, IUserService userService, IDeviceService deviceService, SystemDbContext db) : IPublicService {
    ...
    public async Task<IEnumerable<ItemStockResponse>> ItemStockAsync(string itemCode, string warehouse) {
        var response = (await adapter.ItemStockAsync(itemCode, warehouse)).ToArray();
        var packagesStatuses = new[] { PackageStatus.Active, PackageStatus.Locked };
        var packages = await db.PackageContents
        .Include(v => v.Package)
        .Where(v => v.ItemCode == itemCode && v.WhsCode == warehouse && packagesStatuses.Contains(v.Package.Status))
        .OrderBy(v => v.Package.CreatedAt)
        .ToArrayAsync();
    ...
    public async Task<IEnumerable<ItemBinStockResponse>> ItemBinStockAsync(string itemCode, string whsCode) {
        var response = (await adapter.ItemBinStockAsync(itemCode, whsCode)).ToArray();
