...
public class UpdateLineParameter {
    ...
    public UpdateLineReturnValue Validate(Data data) {
        ...
        if (!CloseLine.HasValue && !QuantityInUnit.HasValue) 
            return (UpdateLineReturnValue)data.GoodsReceiptData.ValidateUpdateLine(this);
        
        if (string.IsNullOrWhiteSpace(UserName))
            throw new Exception("A supervisor password is required to update line!");
        if (!Data.ValidateAccess(UserName, out int empID, out _))
            return UpdateLineReturnValue.SupervisorPassword;
        if (!Global.ValidateAuthorization(empID, Authorization.GoodsReceiptSupervisor))
            return UpdateLineReturnValue.NotSupervisor;
