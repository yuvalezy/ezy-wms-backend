-- declare @ID int = 20;
-- declare @BarCode nvarchar(255) = '1234567890123';
...
select Case 
    ...
    When T2."U_Type" = 'S' and not exists(
        select 1
        from "@LW_YUVAL08_GRPO3" X0
                 left outer join POR1 X1 on X1."DocEntry" = X0."U_DocEntry" and X1."ObjType" = X0."U_ObjType" and X1."ItemCode" = @ItemCode and X1."WhsCode" = @WhsCode
                 left outer join PCH1 X2 on X2."DocEntry" = X0."U_DocEntry" and X2."ObjType" = X0."U_ObjType" and X2."ItemCode" = @ItemCode and X2."WhsCode" = @WhsCode
        where X0.U_ID = @ID and (X1."LineNum" is not null or X2."LineNum" is not null)
    ) Then -6
